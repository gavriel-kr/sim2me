    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="container mx-auto max-w-3xl px-4 py-10">
        ...
        <nav className="mb-6 flex items-center gap-2 text-sm text-gray-500" aria-label="breadcrumb">
          <Link href={`${prefix}/`} className="hover:text-emerald-600">Home</Link>
          <span>/</span>
        ...
        <header className="mb-8">
          <h1 className={`text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl leading-tight ${isRTL ? 'text-right' : ''}`}>
            {article.title}
          </h1>
        ...
        {/* Article body */}
        <div
          className={`prose prose-gray max-w-none prose-headings:font-bold prose-a:text-emerald-700 prose-a:no-underline hover:prose-a:underline prose-img:rounded-xl prose-table:text-sm ${isRTL ? 'text-right prose-headings:text-right' : ''}`}
          dir={isRTL ? 'rtl' : 'ltr'}
