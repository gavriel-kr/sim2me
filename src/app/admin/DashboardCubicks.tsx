...
export function DashboardCubicks({ stats }: { stats: CubickStat[] }) {
  ...
  return (
    <div className="mt-8 w-full max-w-full overflow-x-hidden">
      ...
      <div className="grid w-full max-w-full min-w-0 grid-cols-1 gap-4 overflow-x-hidden sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8">
        {orderedVisible.map((label) => {
          ...
          return (
            <div
              ...
              className={`flex min-w-0 max-w-full cursor-grab items-center gap-2 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm active:cursor-grabbing sm:p-5 ${isDragging ? 'opacity-60' : ''} ${isDropTarget ? 'ring-2 ring-emerald-400 ring-offset-2' : ''}`}
            >
              ...
              <div className="flex-1 min-w-[4rem]">
                <p className="text-xl font-bold text-gray-900 sm:text-2xl break-all">{stat.value}</p>
